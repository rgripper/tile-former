...
The relevant axes are those already computed by Stage 4:
...
CA runs after patch-level axis values are computed (Stages 3–4) and after biome selection (Stage 6), before the tile-level modifier pass (Stage 8). The justification check reads Stage 4 patch axes (drainage, altitude), which are available at this point.

---

## Implementation Notes

Fully implemented in `stage7_caSmoothing()` in `tileGenerator/stage7_caSmoothing.ts`.
...
- Justification check compares out-of-range distance on drainage and altitude axes.

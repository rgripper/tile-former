...
export default function kmeans(
  ...
  if (!init_cent) {
    ...
    while (cents.length < k) {
      let idx: number = Math.floor(random() * data.length);
      if (!def_indexes[idx]) {
    ...
    cents = Cluster.k_means(data, k, random);
  } else if (init_cent === "kmeans++") {
    cents = Cluster.k_means_pp(data, k, random);
  } else {
...
class Cluster {
  ...
  // K-means++ initial centroid selection
  static k_means_pp(data: Vectors, k: number, random: () => number): Centroids {
